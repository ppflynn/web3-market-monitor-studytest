...
这是一个加密货币行情监控项目，包含 Spring Boot 后端、Python 数据采集脚本、Vue 3 前端、Redis 缓存和 Docker Compose 一键部署配置。
...
一键构建并启动 MySQL、Redis、后端、采集脚本和前端：
...
```text
MySQL -> Redis -> Python collector -> Spring Boot API -> Vue frontend
```
